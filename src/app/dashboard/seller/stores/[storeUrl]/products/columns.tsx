...
import { StoreProductType } from "@/types/StoreProduct";
            ...
            {row.original.variants.map((variant) => (
              <div key={variant.id} className="flex flex-col gap-y-2 group">
                <div className="relative cursor-pointer">
                  <Image
                    src={
                      variant.images[0]?.imageUrl || "/placeholder-image.png"
                    }
                    alt={`${row.original.name}image`}
                    width={1000}
                    height={1000}
                    className="min-w-72 max-w-72 h-80  object-cover rounded-md shadow-2xl"
                  />
                  <Link
                    href={`/dashboard/seller/stores/${row.original.store.url}/products/${row.original.id}/variants/${variant.id}`}
                  >
                    <div className="w-full h-full absolute top-0 left-0 bottom-0 right-0 rounded-sm z-10 bg-black/50 bg-opacity-50 transition-all  hidden group-hover:block">
