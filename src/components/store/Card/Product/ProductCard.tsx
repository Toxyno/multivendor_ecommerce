...
const ProductCard = ({ product }: { product: ProductType }) => {
  ...
  // ✅ ALWAYS go to product page WITHOUT sizeId
  // Product page will show price range by default.
  const productUrl = `/product/${slug}?variant=${variantSlug}`;

  // OPTIONAL (if you ever want to deep link to a specific size):
  // const firstSizeId = sizes?.[0]?.id;
  // const productUrlWithSize = firstSizeId
  //   ? `/product/${slug}/${variantSlug}?sizeId=${firstSizeId}`
  //   : productUrl;
    ...
    <div>
      <div className="group w-48 sm:w-56.25 relative transition-all duration-75 bg-whitye ease-in-out p-4 rounded-t-3xl border border-transparent hover:shadow-xl hover:border-border">
        <div className="relative w-full h-full">
          <Link href={productUrl} className="w-full relative inline-block">
            {/* Images Swiper */}
            ...
            {/* ✅ Price Range on card (no sizeId passed) */}
